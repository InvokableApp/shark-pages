/* glp / glp-free-ignyt-sample / ignyt-sample-confirmation-v2 (fork, parent frozen)
 *
 * FORK of ignyt-sample-confirmation (2026-09-17). The parent is FROZEN, see block.html.
 * THIS FILE IS UNCHANGED FROM THE PARENT except for these notes. The v2 addition, the Facebook
 * DM control, needs no JS at all: it is a plain root-relative link to the DM Redirect step, so
 * it works with JS off and cannot be broken by an unfilled merge field. That is also why the
 * unusable-number branch below is now a soft failure rather than a dead end: when rep_phone
 * holds instruction text the text button goes inert, and the DM route is still there and still
 * works.
 *
 * Loads the shared product-page engine for its scroll reveals, and hardens the one SCRIPTED
 * control on the page: the sms: link that opens the visitor's messaging app with the word SAMPLE
