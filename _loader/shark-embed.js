...
(function () {
  ...
  function load(el) {
    ...
    var cssReady = el.__gate
      ? Promise.all([
          addCss(BASE + "_brand/" + system + "/tokens.css", "tokens-" + system),
          installCss(cssHref, dir),
        ])
      // legacy delivery: block.css as a <link>, its @imports left to resolve serially, and
      // nothing waits on any of it.
      : (addCss(BASE + "_brand/" + system + "/tokens.css", "tokens-" + system),
         addCss(cssHref, path + "-css"), Promise.resolve());
    ...
    // gated: markup cannot paint before the rules that lay it out. ungated: inject the
    // moment the markup lands, exactly as before.
    Promise.all(el.__gate ? [htmlReq, cssReady] : [htmlReq])
      .then(function (out) {
        var html = out[0];
