(function () {
/* NUEVA / n-your-marketing-links / n-marketing-links-v2  -  THE CONFIG
 *
 * ⚠️ EVERY SLUG BELOW WAS READ OFF THE SNAPSHOT ACCOUNT sKmJ8BrXPCYR2g7aYRX0 ON
 * 2026-09-24, not copied forward. The v1 hub's four cards were verified against
 * SHARK BETA in September and the snapshot is a later, deliberately smaller
 * build: two of those four funnels (Snow Slim Quiz, Free Revive Sample) are
 * among the four campaigns held out of the product, so both cards were dead
 * links, and The No Crash Plan, which IS in the snapshot, had no card at all.
 * A wrong slug here does not 404, it serves the catch-all with a 200
 * (MARKETING-LINKS-PAGE-SOP §3), so re-measure rather than assume.
 *
 * Link model: one domain custom value plus a fixed slug per funnel, the same
 * mechanism Conectiv and Vital use. Nueva's funnels all sit on one domain.
 * Re-measure before a rep install: one rep on two domains breaks it for that
 * rep. → MARKETING-LINKS-PAGE-SOP §2.
 */
var SYS = {
  ...
  var root = document.querySelector('.sk-nueva-n-your-marketing-links-n-marketing-links-v2');
  if (!root) return;
  ...
  /* The rep's own copy when their domain is set, and NOTHING otherwise: Nueva
     has no central nuevashark.com mirror to fall back to, so an item carries no
     `howto`. On a SNAPSHOT the domain custom value holds its onboarding
     instruction text ("Enter YOUR OWN funnel domain..."), which would otherwise
     render as https://Enter YOUR OWN funnel.../n-side-hustle-how-to on every
     card, so the value has to look like a hostname before it is trusted. The
     training rows are therefore empty in the snapshot, which is correct: the
     page they point at is the buyer's own copy, which does not exist yet. */
  ...
  /* ---------- composed links ----------
     Ported back in from _shared/links-hub/v1, which is where Nueva's hub lived
     before this fork. The Conectiv donor has no equivalent and did not need one.

     Nueva hangs every product URL off ONE identifier the rep owns: attribution
     on nuevalife.com is carried entirely by the subdomain, so all twenty-eight
     product links are the same string with a different tail. Holding twenty-eight
     custom values for that would be twenty-eight chances for a rep to paste a
     link that silently pays someone else, because nuevalife.com does not error
     on a wrong username, it just drops the sponsor.

     A list item carries `path` instead of `cv`/`url`. An empty identifier
     composes NOTHING, deliberately: a half-built URL with the subdomain missing
     resolves to a real page with no sponsor on it, so rendering it would be
     worse than rendering no link at all. */
  ...
  /* ---------- link building ----------
     Two models across the fleet, and the difference is deliberate.
       ...
       slug : one domain custom value plus a fixed slug per funnel. Vital and
              Nueva installs are uniform, so the slugs hold.
  ...
  /* gateCv: render this funnel ONLY on accounts where that custom value is filled.
     This block is ONE file served to every Nueva buyer, so a new entry would otherwise
  ...
  /* ---------- the "2 minute customer video" row on the promote screen ----------
     ...
     Nueva that page is the What Is Nueva funnel, so the row is built by
  ...
  if (twoMin) {
    var tmv = funnelUrl({ slug: 'n-what-is-nueva' });
    twoMin.innerHTML = tmv
      ? '<button class="sk-copy" type="button" data-copy-label="Copy the page link" data-copy="' + tmv + '">' +
        ...
        '<div class="sk-url"><span class="sk-url-text">' + tmv.replace(/^https?:\/\//, '') + '</span>' +
          '<a class="sk-open" href="' + tmv + '" target="_blank" rel="noopener" aria-label="Open your What Is Nueva page">' + icon('out', 1.8) + '</a></div>'
  ...
  (function () {
    var ICON = 'https://invokableapp.github.io/shark-pages/_brand/nueva/';
    ...
    head('meta', { name: 'apple-mobile-web-app-title', content: 'Nueva Shark' });
    head('meta', { name: 'theme-color', content: '#0ABAB5' });
  })();
