(function () {
  ...
  /* ---------- add to home screen ----------
     Deliberately placed ABOVE the domain gate below: that gate returns early when the
     rep has no domain yet, and everything after it stops running.
     ...
     CAN THIS BE AUTOMATIC? Only partly, and only on Android.
       ...
       Android: Chrome MAY fire `beforeinstallprompt`, which is a real one tap
              install. It is not guaranteed, so it is treated as a bonus: when it
              fires the button installs directly and the sheet never opens. */
  (function () {
    ...
    var VIDEO = {
      ios:     { src: "https://assets.cdn.filesafe.space/k5tyIG2Q85sUQ1RlSxBo/media/6a85cfbf5113a1715fd4f0fe.mp4", hint: "(1 minute video)" },
      android: { src: "https://assets.cdn.filesafe.space/k5tyIG2Q85sUQ1RlSxBo/media/6a85cfbf005891114d29ddea.mp4", hint: "(2 minute video)" }
    };
    ...
    function paint(os) {
      ...
      // preload="none" keeps even that at zero bytes until the rep presses play.
